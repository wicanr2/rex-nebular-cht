...
- [HARD] **中文走文字層 → 生命週期跟英文不同：「只畫一次就不管」的畫面必須自己補畫。**
  英文畫進 320 surface 由引擎自己管；中文畫進文字層，由 dirty 清除機制管，
  而那個機制假設「scene 區的文字每幀重畫」。凡是跑完就停住不再重畫的畫面
  （主選單、`GameDialog` 全螢幕對話框），中文被清掉之後沒人補。
  **本專案已踩三次**：UI 區、難度選單、主選單。
  **[HARD] 驗收方式要跟著改**：這類 bug 在「啟動、等、截圖」的靜止流程下 **100% 測不到**
  （沒有 dirty 就不會清）。截圖前要先大量移動滑鼠製造 dirty ——
  主選單那次就是 Linux 靜止截圖過關、wine 截圖露出英文，同一顆 binary 兩種結果。
