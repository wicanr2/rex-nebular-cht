...
目前值 **`6166f3f28ce4`**（124 個檔），對應主選單 baked-art 那一版引擎。

為什麼需要它：包驗收如果只比中文資料的 md5，對「只改引擎、資料沒動」是完全的盲區——
資料全對、包裡卻裝著修正前的引擎，檢查照樣全綠。

**指紋每動一次引擎就會變，所以它只有一個權威來源**：`workplace/tools/apply_patches.sh`
的 `EXPECT_FINGERPRINT` 預設值。三平台的建置都經過它，對不上就 exit 6。
`cht-data/ENGINE.txt` 與這份文件裡的值都是抄過來的副本，改引擎後要一起換
（`b5e9742121ea` 是 baked-art 之前那一版，已作廢）。

> **更早的 `bc70b3a26fd0` 是另一種作廢。** 它記在最初的 `ENGINE.txt` 裡，宣稱的演算法與現在這支相同，
> 但**用它自己記載的演算法重現不出來**，而產生它的腳本沒有留下。
