...
### [雷·必看] 文字層的東西要「持續重畫」，不是「畫一次」

主選單中文化做完，Linux 下截圖是中文，**同一顆 binary 打包成 Windows 包、
用 wine 跑，截到的是英文**。

先確認不是資料或建置問題：包內 `rex_cht.tsv` 4562 筆、有 6 筆 `menu:`、
`ENGINE.txt` 指紋一致、`grep -a "menu:%d" scummvm.exe` 也在。
再開 `--debuglevel=2` 實測，`[CHT] 選單項 N 取色 index=236` 五行都印出來了 ——
**`drawChtLabels()` 有跑，中文有畫**。

根因跟難度選單同一個：文字層的 dirty 清除。差別只在觸發條件 ——

| | 畫面狀態 | 結果 |
|---|---|---|
| Linux 截圖 | 沒碰滑鼠，畫面全靜止，沒有 dirty rect | 中文留著 |
| wine 截圖 | 有 dirty（游標處理不同） | 中文被清掉，露出下面的英文 sprite |

`MainMenu::doFrame()` 跑完動畫就 `if (_menuItemIndex == 6) return;`，之後不再重畫任何東西。
中文畫在文字層，被清掉就沒人補。

**修法**：把「算座標與字色」和「畫」拆開 —— 算一次存進 `_chtLabels`，
`doFrame()` 停在結束狀態時每幀呼叫 `redrawChtLabels()` 補畫。

**驗證方式要跟著改**：原本的截圖驗收是「啟動、等、截圖」，畫面全靜止，
**這個 bug 在那個流程下 100% 測不到**。改成截圖前先大量移動滑鼠製造 dirty，
才是真正的玩家情境。

> **通則**：中文走文字層 ＝ 它的生命週期跟原本的英文不同。英文畫進 320 surface
> 由引擎自己管，中文畫進文字層由 dirty 清除機制管，而那個機制假設「每幀重畫」。
> **凡是「只畫一次就不管」的畫面（主選單、全螢幕對話框），中文都要自己負責補畫。**
> 這一條在本專案已經踩到三次：UI 區（§4-final 的 dirty 驅動）、難度選單、主選單。
